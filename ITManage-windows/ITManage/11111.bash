...
def chageFile():
	for Source_File_List in tqdm.tqdm(os.listdir(Source_Path)):
		time.sleep(0.1)
		...
		if re.search(r"\s+",Finish_Source_File_List):
			Final_File = Final_Path + '\\' + Finish_Source_File_Listb.strip()
		else:
			Final_File = Final_Path + '\\' + Finish_Source_File_Listb
		#准备开始拷贝文件
		#如果目的地目的文件存在则删除
