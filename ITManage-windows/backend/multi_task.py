...
class MultiTask(object):
    ...
    def interactive(self):
        ...
        while True:
            try:
                raw_instruction = input("\033[33;1m[spawn mode]:\033[0m").strip()
    ...
    def __run_cmd(self,msg):
        if self.__parse_argv(msg):
            ...
            if task_host_list:
                while True:
                    user_choice = input("Are you sure you want to run [\033[33;1m%s\033[0m] command on above hosts？（Y/N）" % msg[msg.index('-cmd')+1]).strip()
...
def run_task(task_info,cmd,lock):
    ...
    try:
        s.connect(ip,port,username,password,timeout=5)
