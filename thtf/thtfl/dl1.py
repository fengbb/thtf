#-*- coding: utf-8 -*-
# -*- coding:utf-8 -*-
import socket, threading, re, select, urllib.parse, pdb
lock = threading.Lock()
BUFFER=8192
def HostPort(data):
    index = data.find('\n')
    ...
    host = urllib.parse.urlparse(header['url'])[1]
    print (host)
    port = '80'
    #if hostname.find(':') > 0:
    ...
    if  host.find(':') > 0:
        host,port = urllib.parse.urlparse(header['url'])[1].split(':')
    else:
        host =  urllib.parse.urlparse(header['url'])[1]
    port = int(port)
