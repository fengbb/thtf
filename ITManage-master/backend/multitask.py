    ...
    try:
        if bind_host.host_user.auth_method == 'ssh-password':
            s.connect(bind_host.host.ip_addr,
                      ...
                      timeout=5)
            print (bind_host.host.ip_addr,int(bind_host.host.port),bind_host.host_user.username, bind_host.host_user.password)
        else:
        ...
        #result1 = result[0].decode()
        ###现在执行结果有问题，更改
        #cmd_result = lambda x:len(x.strip())>0,result[0]
        cmd_result = result[0].decode()

        #cmd_result = str(tuple(result)).strip()[0]
        #print ('####'+cmd_result)
        res_status = 'success'
        s.close()
